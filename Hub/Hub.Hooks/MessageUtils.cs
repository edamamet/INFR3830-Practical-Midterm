...
namespace Hub.Hooks {
    public static class MessageUtils {
        public static Message CreateText(Guid senderId, string content) =>
            new Message(MessageType.Text, senderId, DateTime.UtcNow, SerializeText(content));
        ...
        public static string DeserializeText(this Message message) {
            if (message.Header == MessageType.Text) return Encoding.UTF8.GetString(message.Content);
            
            Console.WriteLine("Cannot deserialize a non-text message into text.");
            return string.Empty;
        }
        static byte[] SerializeGuid(Guid guid) {
            return guid.ToByteArray();
        }
        public static Guid DeserializeGuid(this Message message) {
            if (message.Header == MessageType.Registration) return new Guid(message.Content);
            
            Console.WriteLine("Cannot deserialize a non-guid message into a guid.");
            return Guid.Empty;
        }
        ...
        public static (float, float, float) DeserializePosition(this Message message) {
            if (message.Header != MessageType.Position) {
                Console.WriteLine("Cannot deserialize a non-position message into a position.");
                return (0, 0, 0);
            }
